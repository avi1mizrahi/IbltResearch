...
extern "C" {
  ...
  */
  void pyblt_set_parameter_file(const char * filename){
  ...
  unsigned long pyblt_fixed_size(size_t valuesize, size_t ibltSize, size_t numHashes) {
    return (unsigned long) new IBLT(valuesize, ibltSize, numHashes);
  }
  unsigned long pyblt_normal(size_t valuesize, size_t m, size_t numHashes) {
    return (unsigned long) new IBLT(valuesize, m, numHashes, IBLT::Normal());
  }

  int getNumHashes(unsigned long ptr) {
    IBLT* g = reinterpret_cast<IBLT *>(ptr);
    return g->numHashes;
  }

  int getIbltLength(unsigned long ptr) {
    IBLT* g = reinterpret_cast<IBLT *>(ptr);
    return g->hashTableSize();
  }

  size_t getIbltSize(unsigned long ptr) {
    IBLT* g = reinterpret_cast<IBLT *>(ptr);
    return g->bytesize();
  }

  size_t getEntrySize(size_t valuesize) {
    return IBLT::entrybytesize(valuesize);
  }
