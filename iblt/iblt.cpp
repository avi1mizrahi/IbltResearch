...
/// \return theoretical size of entry in bytes
///         (i.e. excluding implementation specific optimizations and overhead)
size_t IBLT::entrybytesize(size_t _valueSize) {
    return sizeof(HashTableEntry) - sizeof(HashTableEntry::valueSum) + _valueSize;
}

/// \return theoretical size in bytes
///         (i.e. excluding implementation specific optimizations and overhead)
size_t IBLT::bytesize() const {
   return hashTableSize() * entrybytesize(valueSize);
}

int IBLT::hashTableSize() const {
...
IBLT::IBLT(size_t _expectedNumEntries, size_t valueSize)  :
 valueSize(valueSize) {
...
IBLT::IBLT(size_t _expectedNumEntries, size_t valueSize, float hedge, size_t _numHashes) :
    valueSize(valueSize) {
...
/*IBLT::IBLT(size_t _expectedNumEntries, size_t _valueSize, size_t ibltSize) :
 valueSize(_valueSize) {
  std::pair<int,double> params = OptimalParameters(_expectedNumEntries);
  numHashes = params.first;
  // Ignore params.second (hedge)
  size_t nEntries = ibltSize / entrybytesize(valueSize);
  // ... make nEntries exactly divisible by numHashes
...
IBLT::IBLT(size_t valueSize, size_t ibltSize, size_t numHashes) :
...
IBLT::IBLT(size_t valueSize, size_t nEntries, size_t numHashes, IBLT::Normal unused) :
        valueSize(valueSize), numHashes(numHashes), hashTable(nEntries) {
}
...
{
    IBLT peeled = *this;
    peeled.peelEntries(positive, negative);
                       ...
                       std::set<std::pair<uint64_t,std::vector<uint8_t> > >& negative) 
{
    ...
    do {
        ...
        for (auto & entry : hashTable) {
            ...
            auto& set = entry.count == 1 ? positive : negative;
            set.insert(std::make_pair(entry.keySum, entry.valueSum));
